...
class LocalesTests(unittest.TestCase):
    ...
    # @unittest.skip('I skip it')
    @wrappers.write_log_and_video(videos_dir)
    def test_es_button_tips_locale(self):
        ''''''
        pyautogui.moveTo(5, 5, 1)
        '''choose language'''

        if not browser.monosearch_10('es_language_selected.png', samples_dir):
            browser.change_lanuage('es_language_selected.png', samples_dir)
            time.sleep(40)
        ...
        '''get lists of buttons and locales'''
        self.buttons = os.listdir('{}{}'.format(os.getcwd(), es_buttons_dir))
        self.locales = os.listdir('{}{}'.format(os.getcwd(), es_locales_dir))

        '''check locales when focus to buttons'''
        for button, locale in zip(self.buttons, self.locales):
            button_coord = browser.monosearch_10(button, es_buttons_dir)
            with self.subTest():
                self.assertTrue(button_coord, msg="{} not finded".format(button))
            if button_coord:
                browser.move_mouse_to(button_coord)
                with self.subTest():
                    self.assertTrue(browser.monosearch_10(locale, es_locales_dir),
                                    msg="{} not finded".format(locale))
